...
export default function QuizPage(): ReactElement {
  ...
  const fetchQuizQuestions = async (): Promise<void> => {
    ...
    try {
      ...
      const response: Response = await fetch(
        `http://localhost:3030/api/questions/${selectedQuiz}/1`
      );
  ...
  return (
    ...
    <div
      className={
        'flex min-h-[calc(100vh-4rem)] flex-col items-center justify-center bg-gradient-to-b from-blue-300 to-blue-800 p-6 text-black caret-transparent'
      }
    >
      {/* SELECTED QUIZ INDICATOR */}
      {quizTitle && <div className={'h-fit w-fit p-2 text-2xl font-bold'}>{quizTitle}</div>}
        ...
        <div>
          {quizzes.map((quiz: Quiz, index: number) => (
            <button
              key={index}
              className={
                'm-4 h-12 w-fit cursor-pointer rounded-lg bg-amber-300 px-4 shadow-xl transition hover:bg-amber-200 active:bg-amber-400'
              }
      ...
      <button
        ...
        disabled={loading}
        className='mb-6 rounded-lg bg-blue-600 px-6 py-3 text-white transition hover:bg-blue-700 active:bg-blue-600 disabled:opacity-50'
      >
        {loading ? 'Loading...' : 'Start Quiz'}
      </button>

      {/* DISPLAY ERROR MESSAGE */}
      {error && <p className='text-center font-bold text-red-300'>{error}</p>}
